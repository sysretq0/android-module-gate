...
holds the snippet: init alone writes (mode/add/remove via init.rc +
load-list.sh); shell reads (monitoring only, no write path by design);
never `untrusted_app`). Apply via
